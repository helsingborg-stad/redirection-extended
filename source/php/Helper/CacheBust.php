...
namespace RedirectionExtended\Helper;
...
class CacheBust
{
    ...
    {
        $jsonPath = REDIRECTIONEXTENDED_PATH . apply_filters('RedirectionExtended/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . REDIRECTIONEXTENDED_PATH . ' and run gulp. See '. REDIRECTIONEXTENDED_PATH . 'README.md for more info.</div>';
        }
    }
}
